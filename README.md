...
Остальные листы той же книги (`smazki`, `nipol`) лежат рядом. У НИПОЛ в таблице нет URL фото, поэтому `<picture>` там не из чего собрать. Yard76 — отдельная выгрузка, не больше 500 карточек: `<picture>` только там, где в строке была ссылка на jpg/png.
...
В исходной таблице нет фото у НИПОЛ (0 из 43). В YML у этих офферов не будет `<picture>`, пока в лист не попадут прямые ссылки на jpg/png/webp.
